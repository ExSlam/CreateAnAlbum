...
Create An Album ships Cormorant Garamond, Cinzel, Cyberthrone, and Allura as **native Unity font assets** in `AlbumFonts/createanalbum_fonts`. Version 4.2.2 loads that AssetBundle with `AssetBundle.LoadFromFile` and resolves the four stable packaged font keys directly to `UnityEngine.Font` objects. The bundle must be built for `StandaloneWindows64` with Unity **2019.4.23f1**, matching Idol Manager. The recommended addressable names are `cormorant_garamond`, `cinzel`, `cyberthrone`, and `allura`; the loader also falls back to matching the original TTF filenames inside a bundle built without addressable names.

The packaged fonts now use the same category of runtime object as Idol Manager's own imported fonts. Cover titles and optional group names are rendered with ordinary `UnityEngine.UI.Text`; CAA no longer privately registers packaged TTF files, rasterizes text with Win32 GDI, or substitutes sprite text for a failed custom face. The four historical Windows choices (Georgia, Times New Roman, Arial Black, and Segoe Script) remain available when Unity can load the already-installed family through `Font.CreateDynamicFontFromOSFont`.

Loose `CustomFonts/*.ttf` / `.otf` loading is intentionally disabled in 4.2.2. Idol Manager's Unity runtime does not expose the Editor `TrueTypeFontImporter`, and the previous CAA private-registration/GDI path did not provide reliable custom-font rendering. An existing `CustomFonts` folder is left untouched and only produces a diagnostic warning. Additional native fonts should be imported into `createanalbum_fonts` instead.

Version 4.1.1 made album backgrounds directory-driven. The shared background catalog scans `AlbumBackgrounds` recursively for `.png`, `.jpg`, and `.jpeg` files, so users can add images and future releases can ship more backgrounds without code changes. The Cover Designer exposes every discovered image through a padded horizontal thumbnail scroller, and persisted albums use stable relative-path background keys so adding files later does not remap existing covers. Legacy numeric background indexes remain migration-compatible.
