...
Version 4.2.0 uses unified **schema v4** persistence for albums, the 14-day chart anchor, release metadata, once-only debut reward state, active production, and historical member portrait descriptors. The descriptors contain idol metadata and texture-asset IDs only; Create An Album intentionally does **not** store rendered cover images or portrait PNGs.
